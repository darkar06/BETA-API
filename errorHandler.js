function errorHandler ( error, req, res, _ ){
  ...
  const errors = {
    ...
    //empty data
    EMPTY_PASSWORD:()=>          res.status( 401 ).json({ error: "necesita proporcionar su contraceña para poder hacer la accion" }),
    ...
    INVALID_LOGIN_PASSWORD:()=>  res.status( 401 ).json({ error: "la contraseña proporcionado no es valida, verifique si hay algo erroneo en la ecritura del mismo" }),
  ...
  return typeof errors[error.name] == "function" 
    ?errors[error.name]()
